...
/vault bind <system> sso <provider> [<url>]        # 2. 声明 system 走 SSO
```
...
| `/vault bind <sys> basic '<user>' '<pass>'` | 绑定 basic 认证 |
| `/vault bind <sys> bearer '<token>'` | 绑定 bearer 认证 |
| `/vault bind <sys> sso <provider> [<url>]` | 绑定 SSO 认证 |
| `/vault bind <provider> basic '<user>' '<pass>'` | 存 SSO 登录账密 |
| `/vault unbind <sys>` | 解绑系统 |
| `/vault list` | 查看状态及系统绑定 |
| `/vault providers` | 列出内置 SSO providers |
