...
- **SSO 会话**：支持浏览器自动登录 SSO 保护的系统，cookie 加密缓存，agent 调用时自动注入
...
> SSO 功能额外依赖 Playwright：`pip install playwright && playwright install chromium`
...
对于走企业 SSO 的系统（如 devops、pms 共享同一个 SSO 登录），插件可自动完成浏览器登录并缓存 cookie。

**配置**：在 `config.yaml` 的 `plugins.entries.hermes-credential-vault` 下声明：
- `systems.<name>.auth: sso_cookie` + `systems.<name>.sso_provider: <provider_name>`
- `sso_providers.<provider_name>` 配置登录 URL / 表单选择器 / cookie 域

**使用流程**：
```
/vault bind devops basic '<user>' '<pass>'    # 只绑定 system，不绑定 provider
/vault sso-login devops                        # 触发浏览器登录，cookie 存入 vault
/vault sso-status                              # 查看所有 SSO system 的会话状态
/vault sso-logout devops                       # 删除本地 session（同 provider 的所有 system 都会受影响）
```

**共享 session**：多个 system 引用同一个 provider 时，一次 `sso-login` 即可覆盖所有关联 system。
